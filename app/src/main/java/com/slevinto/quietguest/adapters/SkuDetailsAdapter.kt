 ...
 */
package com.slevinto.quietguest.adapters
...
import com.slevinto.quietguest.R
...
open class SkuDetailsAdapter : RecyclerView.Adapter<SkuDetailsAdapter.SkuDetailsViewHolder>() {
    ...
    inner class SkuDetailsViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        private fun onDisabled(enabled: Boolean, res: Resources) {
            if (enabled) {
                itemView.apply {
                    setBackgroundColor(res.getColor(R.color.colorAccentLight, null))
                    sku_title.setTextColor(res.getColor(R.color.colorText, null))
                    sku_description.setTextColor(res.getColor(R.color.colorText, null))
                    sku_price.setTextColor(res.getColor(R.color.colorText, null))
                    sku_image.colorFilter = null
                }
            } else {
                itemView.apply {
                    setBackgroundColor(res.getColor(R.color.textDisabledHint, null))
                    val color = res.getColor(R.color.imgDisableHint, null)
